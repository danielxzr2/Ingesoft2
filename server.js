...
const server = http.createServer(async (req, res) => {
  ...
  if (method === 'PUT' && url.startsWith('/tasks/')) {
    ...
    // TODO 3: Verificar que task.userId === usuario.userId
    ...
    // TODO 5: Retornar la tarea actualizada
  ...
  if (method === 'DELETE' && url.startsWith('/tasks/')) {
    ...
    if (taskIndex === -1)
      return send(res, 404, { error: 'Tarea no encontrada' });
    // TODO 3: Verificar que task.userId === usuario.userId
    if (db.tasks[taskIndex].userId !== usuario.userId)
      return send(res, 403, { error: 'No autorizado' });
    // TODO 4: Eliminar la tarea del arreglo
    db.tasks.splice(taskIndex, 1);
    // TODO 5: Retornar 204 sin body
    res.writeHead(204);
    return res.end();
  }
